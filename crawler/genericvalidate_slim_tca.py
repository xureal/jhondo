def gvalidate(filename, filedir, creds):
	...
	sitelistSheet = sitelistFile.get_worksheet(9)
	...
	setOfObjective = set(sitelistSheet.col_values(2))
	
	#Collect list of format from the google sheet and put it into a set
	setOfFormat = set(sitelistSheet.col_values(4))
	
	#Collect list of channel from the google sheet and put it into a set
	setOfChannel = set(sitelistSheet.col_values(6))
	
	#Collect list of audience from the google sheet and put it into a set
	setOfAudience = set(sitelistSheet.col_values(25))	
	...
	for i in range (12,count,1):
		try:
			...
			#creative flag for checking creative and url pairs		
			creative_flag = False
			...
			for j in range (1,19,1):
				...
				#Check if publisher is there but other columns are missing
				publisher = sheet.cell(row=i,column=pub_col).value
				size = sheet.cell(row=i,column=size_col).value
				
				
				c1 = True if (sheet.cell(row=i,column=creative1_col).value!=None and sheet.cell(row=i,column=creative1_col).value!='') else False
				c2 = True if (sheet.cell(row=i,column=creative2_col).value!=None and sheet.cell(row=i,column=creative2_col).value!='') else False
				c3 = True if (sheet.cell(row=i,column=creative3_col).value!=None and sheet.cell(row=i,column=creative3_col).value!='') else False
				u1 = True if (sheet.cell(row=i,column=url1_col).value!=None and sheet.cell(row=i,column=url1_col).value!='') else False
				u2 = True if (sheet.cell(row=i,column=url2_col).value!=None and sheet.cell(row=i,column=url2_col).value!='') else False
				u3 = True if (sheet.cell(row=i,column=url3_col).value!=None and sheet.cell(row=i,column=url3_col).value!='') else False
				
				
				
				
				if (publisher!=None and publisher!='') and not (size=='Package' or size=='package'):
					if ((c1 and u1) and j==1):
						creative_flag = True
					elif j==1:
						logFile.write("In row " + str(i) + " the creative 1 / url 1 pair has an error!\n")
						logString+=("In row " + str(i) + " the creative 1 / url 1 pair has an error!\n")
						errorFlag = 1
					if not ((c2 == u2) and creative_flag) and j==1:
						logFile.write("In row " + str(i) + " the creative 2 / url 2 pair has an error!\n")
						logString+=("In row " + str(i) + " the creative 2 / url 2 pair has an error!\n")
						errorFlag = 1
					if not ((c3 == u3) and creative_flag) and j==1:
						logFile.write("In row " + str(i) + " the creative 3 / url 3 pair has an error!\n")
						logString+=("In row " + str(i) + " the creative 3 / url 3 pair has an error!\n")
						errorFlag = 1
					if ((j!=creative1_col and j!=url1_col and j!=creative2_col and j!=url2_col and j!=creative3_col and j!=url3_col and j!=units_col and j!=rate_col) and (sheet.cell(row=i,column=j).value==None or sheet.cell(row=i,column=j).value=='')):
			...
			#Check if it's present in the audiencelist on google sheet
			audience = sheet.cell(row=i,column=audience_col).value
			if audience!=None and audience!='':
				if audience not in setOfAudience:
					print("In row " + str(i) + " the audience - " + audience + " is not valid, please revise it!")
					logFile.write("In row " + str(i) + " the audience - " + audience + " is not valid, please revise it!\n")
					logString+=("In row " + str(i) + " the audience - " + audience + " is not valid, please revise it!\n")
					errorFlag = 1
			...
			# adServer = sheet.cell(row=i,column=21).value
			# if adServer!=None and adServer!='':
				# if adServer not in setOfAdServer:
