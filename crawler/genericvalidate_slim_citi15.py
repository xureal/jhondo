def gvalidate(filename, filedir, creds):
	...
	for i in range (14,count,1):
		try:
			...
			for j in range (1,25,1):
				...
				if cellValue!=None and cellValue!='':
					if cellValue.strip() != cellValue:
						print("In row " + str(i) + " the column - " + str(sheet.cell(row=13,column=j).value) + " has leading or trailing spaces!")
						logFile.write("In row " + str(i) + " the column - " + str(sheet.cell(row=13,column=j).value) + " has leading or trailing spaces!\n")
						logString+=("In row " + str(i) + " the column - " + str(sheet.cell(row=13,column=j).value) + " has leading or trailing spaces!\n")
						errorFlag = 1
				...
				if (publisher!=None and publisher!='') and not (size=='Package' or size=='package'):
					if sheet.cell(row=i,column=j).value==None or sheet.cell(row=i,column=j).value=='':
						if (j==creative2_col or j==url2_col or j==creative3_col or j==url3_col) and ((sheet.cell(row=i,column=creative1_col).value==None or sheet.cell(row=i,column=creative1_col).value=='') and (sheet.cell(row=i,column=url1_col).value==None or sheet.cell(row=i,column=url1_col).value=='')):
							...
							errorFlag = 1
						elif (j!=creative2_col and j!=url2_col and j!=creative3_col and j!=url3_col and j!=units_col and j!=rate_col and j!=cost_col):
							print("In row " + str(i) + " the column - " + str(sheet.cell(row=11,column=j).value) + " is blank!")
							logFile.write("In row " + str(i) + " the column - " + str(sheet.cell(row=11,column=j).value) + " is blank!\n")
							logString+=("In row " + str(i) + " the column - " + str(sheet.cell(row=11,column=j).value) + " is blank!\n")
							errorFlag = 1
			...
			offerType = sheet.cell(row=i,column=creativeoffertype_col).value
			if offerType!=None and offerType!='':
				if offerType not in setOfOfferType:
					print("In row " + str(i) + " the offer type - " + offerType + " is not valid, please revise it!")
					logFile.write("In row " + str(i) + " the offer type - " + offerType + " is not valid, please revise it!\n")
					logString+=("In row " + str(i) + " the offer type - " + offerType + " is not valid, please revise it!\n")
					errorFlag = 1

			#Check creative type column
			#Check if it's present in the creative type list on google sheet
			placementType = sheet.cell(row=i,column=plctype_col).value
			...
			costModel = sheet.cell(row=i,column=costmodel_col).value
			...
			creativeAsset = sheet.cell(row=i,column=creativetype_col).value
			if creativeAsset!=None and creativeAsset!='':
				if creativeAsset not in setOfCreativeAsset:
					print("In row " + str(i) + " the creative type - " + creativeAsset + " is not valid, please revise it!")
					logFile.write("In row " + str(i) + " the creative type - " + creativeAsset + " is not valid, please revise it!\n")
					logString+=("In row " + str(i) + " the creative type - " + creativeAsset + " is not valid, please revise it!\n")
					errorFlag = 1
					
			#Check objective column
			#Check if it's present in the objective list on google sheet
			servingMethod = sheet.cell(row=i,column=servingmethod_col).value
			if servingMethod!=None and servingMethod!='':
				if servingMethod not in setOfServingMethod:
					print("In row " + str(i) + " the serving method - " + servingMethod + " is not valid, please revise it!")
					logFile.write("In row " + str(i) + " the serving method- " + servingMethod + " is not valid, please revise it!\n")
					logString+=("In row " + str(i) + " the serving method - " + servingMethod + " is not valid, please revise it!\n")
					errorFlag = 1
			...
			adType = sheet.cell(row=i,column=adtype_col).value
			...
			target = sheet.cell(row=i,column=target_col).value
			...
			#Check the placement name column
			#Check it to match the special characer regex above
			adDesc = sheet.cell(row=i,column=addesc_col).value
