def gvalidate(filename, filedir, creds):
	...
	sitelistSheet = sitelistFile.get_worksheet(8)
	...
	setOfLanguages = set(sitelistSheet.col_values(11))

	#Collect list of countries from the google sheet and put it into a set
	setOfCountries = set(sitelistSheet.col_values(3))
	...
	setOfCostModel = set(sitelistSheet.col_values(2))
	...
	setOfTagType = set(sitelistSheet.col_values(14))
	...
	setOfObjective = set(sitelistSheet.col_values(4))
	
	#Collect list of format from the google sheet and put it into a set
	setOfDevice = set(sitelistSheet.col_values(19))
	
	#Collect list of channel from the google sheet and put it into a set
	setOfTargetSubType = set(sitelistSheet.col_values(18))
	...
	for i in range (12,count,1):
		try:
			...
			size = sheet.cell(row=i,column=size_col).value
			if size!=None and size!='' and size!='Package' and size!='package' and "VID" not in size and "MULTI" not in size and "Video" not in size and "15S" not in size and "30S" not in size:
			...
			targettype = sheet.cell(row=i,column=targettype_col).value
			if targettype!=None and targettype!='':
				if targettype not in setOfTargetSubType:
					print("In row " + str(i) + " the target type - " + targettype + " is not valid, please revise it!")
					logFile.write("In row " + str(i) + " the target type - " + targettype + " is not valid, please revise it!\n")
					logString+=("In row " + str(i) + " the target type - " + targettype + " is not valid, please revise it!\n")
					errorFlag = 1
